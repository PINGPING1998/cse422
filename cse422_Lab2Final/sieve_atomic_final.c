...
static int sieve_init(void)
{
    ...
    //Check if threads are allocated without error
    if(threads == NULL){
        printk("Error in allocating threads array\n");
        upper_bound = 0;
        num_threads = 0;
        kfree(num_array);
        num_array = 0;
    ...
    for(j = 0; j < num_threads; j++){
        ...
        //check to see if error on creating thread
        if(threads[j] == NULL){
            printk("Error in creating thread\n");
            upper_bound = 0;
            num_threads = 0;
            kfree(num_array);
            num_array = 0;
...
static void sieve_exit(void){
    ...
    for(i = 0; i < upper_bound -1; i++){
        if(atomic_read(&num_array[i]) != 0){
            prime_count++;
            if(prime_count % print_eight != 0){
                printk(KERN_CONT " %d ", atomic_read(&num_array[i]));
            }
            else{
                printk(" %d ", atomic_read(&num_array[i]));
                print_eight = print_eight+8;
            }
        }
        total_elements++;
    }
